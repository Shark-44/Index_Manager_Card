...
async _addComposant() {
    ...
    // Si la pièce est Materiel_mobile → role forcé à appareils
    const role = this._selectedPiece === "Materiel_mobile"
        ? "appareils"
        : getRole(entity);
...
async _updateComposant(c, newEntity) {
    ...
    try {
        this.error = null;
        await api("composant", "POST", { action: "update", piece: this._selectedPiece, role: role, id: c.id, entity: newEntity, type });
        this._editComposant = null;
        await this._getPieces();
    } catch (e) {
...
_renderDetail() {
    ...
    const appareils   = (piece?.appareils   || []).map(c => ({ ...c, role: "appareils" }));

    const formHint = this._newEntity
    ? html`${this._t("role_hint")} <b>${
        this._selectedPiece === "Materiel_mobile" ? "appareils" : getRole(this._newEntity)
    }</b>, ${this._t("type_hint")} <b>${getType(this._newEntity)}</b>`
    : "";
    ...
    <ha-card header="${this._selectedPiece}">
        ...
        ${appareils.length ? html`
            <h4 class="section-title">${this._t("devices")}</h4>
            <div class="composants-grid">
            ${appareils.map(c => this._renderComposant(c))}
            </div>
        ` : ""}
